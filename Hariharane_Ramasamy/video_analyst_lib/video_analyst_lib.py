...
A general comment is to use LOG for python and use log file to write the program status 
like debug, fatal error. This is better way to look at what wrong in the program which 
is missing all along  pythonlog4j
Hariharane Ramasamyu comment ends
'''
...
''' 
Hariharane Ramasamy Comment begins 
Nothing much to comment here as these create/load combination functions
There is one issue while storing output file - there is no defensive programming to check for existence of 
the output file. Due to this it may overwrite the previous file. If a file exists, then an extra name should be 
appended to the file to make it unique prior to dumping the file. 
Hariharane Ramasamy comment ends 
'''
