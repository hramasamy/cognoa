...
''' 
Hariharane Ramasmy comment begins 
The following coefficient are hardcoded probably from training in ds_helper_functions.py will have to be fetched from some place 
where values are refreshed when new training is done. For examples, the coefficient value can be deposited in database, and refereshed 
in a memcached server  which can be fetched here to always yield the correct result
